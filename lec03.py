...
#for a rank deficient matrix, the pseudo inverse is useful to find the coeffs while all other methods fail
#pseudoinverse method takes care of the rank deficiency of the matrix by checking the singular values as it cutoffs for small singular values
#for 1/s we have to set some threshold value to avoid division by zero
#in machine learning applications we use tall matrices where the number of samples is greater than the number of features
#plot data on X[;;1] vs y axis
#also plot regression line with only X[;;1] as feature
#first make X[:, 1] as np.arange between min and max of X[:, 1]
#then calculate the predictions using the coefficients
#when we do a ls fit the error is 
#orthogonal regression is used when we have errors in both x and y
